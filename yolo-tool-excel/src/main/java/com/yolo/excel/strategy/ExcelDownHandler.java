package com.yolo.excel.strategy;
...
import com.yolo.excel.anno.ExcelEnumFormat;
...
public class ExcelDownHandler implements SheetWriteHandler {
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (fields[i].isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (CharSequenceUtil.isNotBlank(converterExp)) {
                    // 如果指定了确切的值，则直接解析确切的值
                    options = CharSequenceUtil.split(converterExp, format.separator(), true, true);
                }
    ...
    private String getExcelColumnName(int columnIndex) {
        ...
        String columnPrefix = columnCircleCount == 0
            ...
            : CharSequenceUtil.subWithLength(EXCEL_COLUMN_NAME, columnCircleCount - 1, 1);
        // 从26一循环内取对应的栏位名
        String columnNext = CharSequenceUtil.subWithLength(EXCEL_COLUMN_NAME, thisCircleColumnIndex, 1);
        // 将二者拼接即为最终的栏位名
